...
<div class="p-4 sm:p-6 lg:p-8">
    <div class="mb-8">
        <a href="{{ route('inventaris.show_grouped', $asetDetail->inventaris) }}" class="inline-flex items-center gap-2 text-sm font-semibold text-gray-600 hover:text-gray-900 mb-4">
                        ...
                        <div>
                            ...
                            <div class="mt-2">
                                <select name="kondisi" id="kondisi" required
                                    ...
                                    <option value="Baik" @if(old('kondisi', $asetDetail->kondisi) == 'Baik') selected @endif>Baik</option>
                                    <option value="Rusak Ringan" @if(old('kondisi', $asetDetail->kondisi) == 'Rusak Ringan') selected @endif>Rusak Ringan</option>
                                    <option value="Rusak Berat" @if(old('kondisi', $asetDetail->kondisi) == 'Rusak Berat') selected @endif>Rusak Berat</option>
                                </select>
                            </div>
                        </div>
                            ...
                            <div class="mt-2">
                                <select name="room_id" id="room_id"
                                    ...
                                    @foreach($rooms as $room)
                                        <option value="{{ $room->id_room }}" @if(old('room_id', $asetDetail->room_id) == $room->id_room) selected @endif>{{ $room->nama_ruangan }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                            ...
                            <div class="mt-2">
                                <select name="penanggung_jawab_id" id="penanggung_jawab_id"
                                    ...
                                    @foreach($users as $user)
                                        <option value="{{ $user->id }}" @if(old('penanggung_jawab_id', $asetDetail->penanggung_jawab_id) == $user->id) selected @endif>{{ $user->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <div class="mt-2">
                                <textarea name="keterangan" id="keterangan" rows="3"
                                    class="block w-full rounded-lg border-0 bg-white/50 py-3 px-4 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-indigo-500 transition-all duration-200"
                                    placeholder="Detail tambahan, catatan perbaikan, dll...">{{ old('keterangan', $asetDetail->keterangan) }}</textarea>
                            </div>
                        </div>
                    </div>
                </div> 
