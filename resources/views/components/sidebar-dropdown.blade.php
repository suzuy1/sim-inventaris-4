@props(['label', 'icon', 'active' => false, 'collapsed' => false])
...
<div x-data="{ open: {{ $active ? 'true' : 'false' }} }" class="space-y-1">
    <button @click="open = !open" 
        ...
        <span x-show="!collapsed" class="flex-1 text-left">{{ $label }}</span>
        <svg x-show="!collapsed" :class="{'rotate-90': open}" class="w-4 h-4 transition-transform duration-200 text-gray-400 group-hover:text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/>
        </svg>
    </button>
    
    <ul x-show="open && !collapsed" x-transition:enter="transition ease-out duration-150" 
